...
function List(props) {
    ...
    const deleteit = (id) => {
        if (window.confirm("Do you really want to delete this record?")) {
            deletes('products/' + id).then(fdata => {
                setData(fdata);
                setReload(!isReload)
                                            ...
                                            data.map((info, index) => {
                                                return <tr key={"row" + index} className="hover:bg-gray-50 transition-colors">
                                                    ...
                                                    <td className="whitespace-nowrap px-6 py-4 text-sm text-gray-500">
                                                        <Link className="px-6 py-3 bg-green-600 text-white font-medium rounded-lg hover:bg-green-700 shadow-lg transition-all" to={"/products/edit/" + info.id}>Edit</Link>
